...
f32 noise2d(f32 x, f32 y)
{
    ...
    f32 x_frac = x - x_int;
    f32 y_frac = y - y_int;
    i32 s = noise2(x_int, y_int);
    i32 t = noise2(x_int+1, y_int);
    i32 u = noise2(x_int, y_int+1);
    i32 v = noise2(x_int+1, y_int+1);
    f32 low = smooth_inter(s, t, x_frac);
    f32 high = smooth_inter(u, v, x_frac);
...
float noise_inter(float a0, float a1, float w) {
...
float perlin(float x, float y) {
    ...
    ix0 = noise_inter(n0, n1, sx);

    n0 = dotGridGradient(x0, y1, x, y);
    n1 = dotGridGradient(x1, y1, x, y);
    ix1 = noise_inter(n0, n1, sx);

    value = noise_inter(ix0, ix1, sy);
    return value;
}